...
def get_loss_per_key(key: int, csv_dir):
    key_one = glob(f"{csv_dir}/slide_*_key_{key}.csv")
    assert len(key_one) > 0

    oracle_residual = pd.read_csv(f"{csv_dir}/oracle_key_{key}.csv")[
        "pred_residual"
    ]
    ...
    for path in key_one:
        slide_size = int(
            os.path.basename(path).split("_key_")[0].replace("slide_", "")
        )
        df = pd.read_csv(path)

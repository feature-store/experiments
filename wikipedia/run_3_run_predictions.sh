...
do
	...
	do
		...
		do
			for key_policy in  "weighted_round_robin" "round_robin" "random" "weighted_random"
			do
				plan_file=plan-${key_policy}_${event_policy}-${load_shedding_policy}-${model_runtime}-100
				echo $plan_file
				CUDA_VISIBLE_DEVICES=0,1,4 bash ${dpr_dir}/evaluate_retrieval_single_doc_stream.sh $plan_file &

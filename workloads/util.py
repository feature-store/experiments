...
def download_dir(name, source_dir, target_dir):
    ...
    objs = s3.list_objects(
        Bucket="feature-store-datasets", Prefix=f"{source_dir}/{name}"
    )
    if len(objs) == 0:
        logger.msg(
            "Bucket is empty or does not exist",
            parent_dir=s3.list_objects(
                Bucket="feature-store-datasets", Prefix=f"{source_dir}"
            ),
        )
        return None
    ...
    for obj in objs["Contents"]:
        ...
        Path(os.path.split(target)[0]).mkdir(parents=True, exist_ok=True)
        logger.msg("Downloading", from_=key, to_=target)
...
def use_dataset(name, redownload=False):
    ...
    if not os.path.isdir(path) or redownload:
        ...
        return download_dir(
            name, config["aws_dir"] + "/datasets", config["dataset_dir"]
        )
...
def join_queries_features(input_queries_df, input_features_df):
    ...
    for qi in tqdm(range(len(queries_df.index))):
        ...
        while (
            fi + 1 < len(features_df.index)
            and features_df.iloc[fi + 1].timestamp_ms <= ts
            and features_df.iloc[fi + 1].key_id == key
        ):
            fi += 1

        if fi >= len(features_df.index):
            break
        ...
        assert (
            features_df.iloc[fi].timestamp_ms <= ts
            and features_df.iloc[fi].key_id == key
        ), f"Mismatch {fi}/{qi}: {features_df.iloc[fi].timestamp_ms}/{ts}, {features_df.iloc[fi].key_id}/{key}"
...
def join_queries_features_key(input_queries_df, input_features_df, key):
    ...
    queries_df = input_queries_df[input_queries_df["key_id"] == key].sort_values(
        ["timestamp_ms"]
    )
    features_df = input_features_df[input_features_df["key_id"] == key].sort_values(
        ["timestamp_ms"]
    )
